...
class Detector():
    ...
    def __init__(self):
        self.objectList = []
        self.weights = "best.pt"  # 可去官方下载v5 6.0的预训练权重模型
        self.dnn = False
        self.data = "E:\pycharm_space\yolov5\data\hat.yaml"  # 选择你的配置文件(一般为.yaml)
        self.device = select_device(device='')
        self.half = self.device.type != 'cpu'  # 半精度化
        self.predefined_classes = []
        self.imgdir = "E:\\other_space\\ai_material\\auto_jpg"  # 你需要标注的图片文件夹
        self.outdir = "E:\\other_space\\ai_material\\auto_xml" # 你需要保存的xml文件夹
        self.detect_class = "class.txt" # 你需要自动标注的类型
